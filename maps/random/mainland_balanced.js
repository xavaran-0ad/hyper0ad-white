...
let playerAreas;
if (!isNomad()) {
	playerAreas = getSurroundingAreas(playerPositions[1]);
}
...
if (!isNomad()) {
	...
	let auxConstraint = avoidClasses(clPlayer, 20, clForest, 18, clHill, 0);
	let constraint = new AndConstraint([auxConstraint, new AvoidMapBoundsConstraint(7)]);
...
if (!isNomad()) {
	placePlayerFoodBalanced(playerAreas, oFruitBush, null, oMainHuntableAnimal, oSecondaryHuntableAnimal, clFood,
		avoidClasses(clForest, 2, clPlayer, 25, clHill, 1, clMetal, 4, clRock, 4, clFood, 15));
}
